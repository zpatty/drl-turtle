...
class TrackAny:
    ...
    def drop_spot_on_click(self, event, x ,y, flags, userdata):
        ...
        if self.state_for_retrack > 1 and self.add:
            # self.clicks_for_retrack.append([x, y])
            self.add = 0
        if len(self.clicks_for_retrack) >= self.cfg['num_of_clicks_for_detection']:
            self.frame_idx = 0
...
#     while 1:
#         tracker.mission_counter +=1
#         bounding_boxes, masks, saved_frame = tracker.detect_object(frame)
#         status, clicks_for_retrack = tracker.track_object_with_cutie(masks, frame)
#         tracker.clicks_for_retrack = []
#         tracker.state_for_retrack = 0
