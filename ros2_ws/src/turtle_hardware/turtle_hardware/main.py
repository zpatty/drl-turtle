...
from mjc_turtle_robot.controllers.joint_space_controllers import cornelia_joint_space_control_factory
...
# class TurtleRobot(Node, gym.Env):
    ...
    def turtle_mode_callback(self, msg):
        ...
        # elif msg.data == 'SAC':
        #     self.mode = 'SAC'
        # elif msg.data == 'teacher_traj_input':
...
def execute_data_primitive(turtle_node, primitive, num_cycles = 1):
    if primitive != "dwell":
...
def execute_primitive_pos(turtle_node, primitive, t_0, variable = True):
    ...
    if variable:
        ...
        qd, dqd, ddqd = primitive(time.time() - t_0, q)
...
def execute_primitive_vel(turtle_node, primitive, t_0):
    ...
    rclpy.spin_once(turtle_node)
    if turtle_node.mode == 'rest' or turtle_node.mode == 'stop':
        turtle_node.Joints.send_vel_cmd(turtle_node.nq * [0])
        turtle_node.Joints.disable_torque()
        first_time = True
        return
    # check for new trajectory information
    ...
    u = primitive(time.time() - t_0, q_prim[:6])
    ...
    input_history = np.append(input_history[:,1:], ud ,axis=1)
    input_mean = np.mean(input_history, axis = 1)
    # curr = grab_arm_current(input_mean, min_torque, max_torque)
    # print(ud)
...
def main(args=None):
    ...
    sw = 2.0
    joint_space_control_fn = cornelia_joint_space_control_factory(
        kp=turtle_node.Kpv, q_off=q_off, sw=sw
    )
    kp = [1e1]*6
        ...
        while rclpy.ok():
            ...
            try:
                ...
                if turtle_node.mode == 'stop':
                    ...
                    while True:
                        rclpy.spin_once(turtle_node)
                        if turtle_node.mode == 'rest' or turtle_node.mode == 'stop' or turtle_node.voltage < turtle_node.voltage_threshold:
                            turtle_node.Joints.send_torque_cmd([0] *len(turtle_node.IDs))
                            turtle_node.Joints.disable_torque()
                            break

                        primitive = turtle_node.mode
                        execute_data_primitive(turtle_node, primitive, num_cycles)
                    turtle_node.Joints.disable_torque()
                    ...
                    while True:
                        ...
                        execute_primitive_pos(turtle_node, primitive, t_0, True)

                    turtle_node.Joints.send_torque_cmd([0] *len(turtle_node.IDs))
                    turtle_node.Joints.disable_torque()
                elif turtle_node.mode == 'jsms':
                    """
                    Randomly pick a motion primitive and run it 4-5 times
                    """
                    # primitives = ['turnrr']
                    ...
                    while True:
                        rclpy.spin_once(turtle_node)
                        if turtle_node.mode == 'rest' or turtle_node.mode == 'stop' or turtle_node.voltage < turtle_node.voltage_threshold:
                            turtle_node.Joints.send_vel_cmd([0] *len(turtle_node.IDs))
                            turtle_node.Joints.disable_torque()
                            break

                        execute_primitive_vel(turtle_node, joint_space_control_fn, t_0)
                    turtle_node.Joints.send_vel_cmd([0] *len(turtle_node.IDs))
                    turtle_node.Joints.disable_torque()

                elif turtle_node.mode == 'tsms':
                    """
                    Randomly pick a motion primitive and run it 4-5 times
                    """
                    # primitives = ['turnrr']
                    ...
                    while True:
                        ...
                        execute_primitive_vel(turtle_node, primitive, t_0)
                        print(f"[DEBUG] dt: {time.time() - t_last}\n")
                        t_last = time.time()
                    turtle_node.Joints.send_vel_cmd([0] *len(turtle_node.IDs))
                    turtle_node.Joints.disable_torque()

                elif turtle_node.mode == 'ltsms':
                    """
                    Randomly pick a motion primitive and run it 4-5 times
                    """
                    # primitives = ['turnrr']
                    ...
                    print("traj input")
                    qd_mat = mat2np(f'/home/tortuga/drl-turtle/ros2_ws/src/turtle_hardware/turtle_hardware/turtle_trajectory/teacher_traj/qd.mat', 'qd')
                    dqd_mat = mat2np(f'/home/tortuga/drl-turtle/ros2_ws/src/turtle_hardware/turtle_hardware/turtle_trajectory/teacher_traj/dqd.mat', 'dqd')
                    ddqd_mat = mat2np(f'/home/tortuga/drl-turtle/ros2_ws/src/turtle_hardware/turtle_hardware/turtle_trajectory/teacher_traj/ddqd.mat', 'ddqd')
                    tvec = mat2np(f'/home/tortuga/drl-turtle/ros2_ws/src/turtle_hardware/turtle_hardware/turtle_trajectory/teacher_traj/tvec.mat', 'tvec')
                    print("plan plan plan")
                    primitives = ['surface', 'turnrf', 'turnrr', 'straight', 'turnlr']
                    # primitives = ['turnrr']
                    num_cycles = 1
                    ...
                    q_data = np.zeros((turtle_node.nq,1))
                    dq_data = np.zeros((turtle_node.nq,1))
                    ddq_data = np.zeros((turtle_node.nq,1))
                    tau_data = np.zeros((turtle_node.nq,1))
                    ...
                    while 1:
                        if turtle_node.voltage < turtle_node.voltage_threshold:
                            ...
                            scipy.io.savemat(folder_name + "/qd.mat", {
                                'qd': q_data[:, 1:]})
                            scipy.io.savemat(folder_name + "/dqd.mat", {
                                'dqd': dq_data[:, 1:]})
                            scipy.io.savemat(folder_name + "/ddqd.mat", {
                                'ddqd': ddq_data[:, 1:]})
                            scipy.io.savemat(folder_name + "/tvec.mat", {
                                'tvec': time_data})       
                            break
                        rclpy.spin_once(turtle_node)
                        # print("traj 1...")
                        ...
                        if first_time:
                            t_old = time.time()
                            dq = np.zeros((turtle_node.nq,1))
                            ddq = np.zeros((turtle_node.nq,1))
                            dq_data=np.append(dq_data, dq, axis = 1) 
                            ...
                            dq_data=np.append(dq_data, dq, axis = 1) 
                            ddq_data=np.append(ddq_data, ddq, axis = 1) 
                        time_data = np.append(time_data, (time.time()-t_begin)) 
                    turtle_node.Joints.disable_torque()

                else:
                    turtle_node.mode = 'rest'
